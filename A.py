...
# H(N):
def costo_estimado(nodo, nodo_fin):
    x1, y1 = Grafico_utm.nodes[nodo]['x'], Grafico_utm.nodes[nodo]['y']
    x2, y2 = Grafico_utm.nodes[nodo_fin]['x'], Grafico_utm.nodes[nodo_fin]['y']
    distancia_euclidiana = sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2) - 1
...
# ALGORITMO A*
def a_visual(nodo_inicio, nodo_fin):
    ...
    while lista_por_explorar:
        ...
        for nodo_vecino, distancia in vecino(nodo_actual):
            ...
            if nodo_vecino not in costos or nuevo_costo < costos[nodo_vecino]:
                ...
                prioridad = nuevo_costo + costo_estimado(nodo_vecino, nodo_fin)
                lista_por_explorar.append((prioridad, nodo_vecino))
                padres[nodo_vecino] = nodo_actual

    # RECONSTRUCIIÓN FINAL
    camino = []
    nodo = nodo_fin
...
nodo_fin = 8321745919
camino_encontrado = a_visual(nodo_inicio, nodo_fin)
